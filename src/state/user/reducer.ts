...
export interface UserState {
  ...
  // user confirm terms of use
  userTermsOfUseConfirm: boolean

  // user confirm state for >3% slippage warning
  userSlippageConfirm: boolean

  userUnvettedTokenConfirm: boolean

  // "Are you the project developer?" confirmation on click liquidity "Supply"
  userLiquidityDevConfirm: boolean
  ...
  gasTypeByChain: Partial<Record<ChainId, string>>
...
export const initialState: UserState = {
  ...
  userSlippageConfirm: false,
  userTermsOfUseConfirm: false,
  userUnvettedTokenConfirm: false,
  userLiquidityDevConfirm: false,
...
export default createReducer(initialState, (builder) =>
  builder
    .addCase(togglePinToken, (state, action) => {
      if (!state.pinnedTokens) {
        state.pinnedTokens = {}
      }
      if (!state.pinnedTokens[action.payload.chainId]) {
        state.pinnedTokens[action.payload.chainId] = [action.payload.address]
      } else {
        const list = state.pinnedTokens[action.payload.chainId]
        const loweredAddr = action.payload.address.toLowerCase()

        if (list.some((addr) => addr.toLowerCase() === loweredAddr)) {
          state.pinnedTokens[action.payload.chainId] = list.filter((addr) => addr.toLowerCase() !== loweredAddr)
        } else {
          state.pinnedTokens[action.payload.chainId].push(action.payload.address)
        }
      }
    })
    ...
    .addCase(updateUserLiquidityDevConfirm, (state, action) => {
      state.userLiquidityDevConfirm = action.payload.userLiquidityDevConfirm
      state.timestamp = currentTimestamp()
    })
    .addCase(updateUserUnvettedTokenConfirm, (state, action) => {
      state.userUnvettedTokenConfirm = action.payload.userUnvettedTokenConfirm
      state.timestamp = currentTimestamp()
    })
    .addCase(updateUserSlippageConfirm, (state, action) => {
      state.userSlippageConfirm = action.payload.userSlippageConfirm
      state.timestamp = currentTimestamp()
    })
    .addCase(updateUserTermsOfUseConfirm, (state, action) => {
      state.userTermsOfUseConfirm = action.payload.userTermsOfUseConfirm
      state.timestamp = currentTimestamp()
    })
    ...
    .addCase(updateGasPrice, (state, action) => {
      if (!state.gasTypeByChain) {
        state.gasTypeByChain = {}
      }
      state.gasTypeByChain[action.payload.chainId] = action.payload.gasType
    })
    ...
    .addCase(setShowAccountDomain, (state, { payload }) => {
      state.showAccountDomain = payload
    }),
)
