...
export function useMultipleContractSingleData(
  ...
  return useMemo(() => {
    const currentBlockNumber = cache.get(unstable_serialize(['blockNumber', chainId]))
    const temp = results.map((result) => toCallState(result, contractInterface, fragment, currentBlockNumber))
    return temp.length > 0 ? temp : EMPTY_ARRAY // use to reduce extra rerender cause by new emtpy array
