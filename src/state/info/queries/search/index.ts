...
import { getInfoClientByChainId } from 'utils/graphql'
...
const TOKEN_SEARCH = gql`
  query tokens($symbol: String, $name: String, $id: String) {
    asSymbol: tokens(
      first: 10
      where: { symbol_contains: $symbol, totalTransactions_gte: 1 }
    ...
    asName: tokens(
      first: 10
      where: { name_contains: $name, totalTransactions_gte: 1 }
      orderBy: tradeVolumeUSD
      orderDirection: desc
    ) {
      id
    ...
    asAddress: tokens(
      first: 1
      where: { id: $id, totalTransactions_gte: 1 }
      orderBy: tradeVolumeUSD
      orderDirection: desc
    ) {
      id
...
const useFetchSearchResults = (
  ...
  useEffect(() => {
    const search = async () => {
      try {
        const tokens = await getInfoClientByChainId(chainId).request<TokenSearchResponse>(TOKEN_SEARCH, {
